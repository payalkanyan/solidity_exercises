...
contract TicTacToe {
    ...
    */
    ...
    function isWinning(uint8[3][3] memory board) public pure returns (bool) {
        ...
        for (uint8 i = 0; i<3; i++){
            if(board[0][i]!=0 && board[0][i]== board[i][1] && board[i][1] == board[i][2]){
                return true;
            }
        }

        for (uint8 i = 0; i<3; i++){
            if((board[0][0]!=0 && board[0][0]== board[1][1] && board[i][1] == board[2][2]) || (board[2][0]!=0 && board[2][0]== board[1][1] && board[1][1] == board[0][2]))
            {
                return true;
            }
        }
